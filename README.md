...
- **Inventar prüfen** (read-only): Erreichbarkeit, Modell/Generation und aktuelle iLO-/BIOS-/Komponenten-Versionen pro Server
- **Firmware aktualisieren** in drei Modi:
  - **Einzeldatei** (`.fwpkg`)
...
Für gemischte Flotten empfiehlt sich der Modus **Basisverzeichnis**. Darunter wird je Servertyp ein Unterordner mit den `.fwpkg`-Dateien angelegt. Die Zuordnung erfolgt automatisch anhand des am iLO ausgelesenen Modells:

```
Firmware\                 ← Basisverzeichnis (im Tool auswählen)
├── DL360 Gen10\          ← Typ-spezifisch (Modell-Teilstring, längster Treffer gewinnt)
│   ├── bios_...fwpkg
│   └── ilo5_...fwpkg
├── DL380 Gen11\
│   └── ...
└── Gen10\                ← generischer Fallback je Generation (auch "g10" möglich)
    └── ...
```

- Zuordnung zuerst per **Modell-Teilstring** (z. B. Ordner `DL360 Gen10` für Modell „ProLiant DL360 Gen10“), sonst **Generation** als Fallback (`Gen10`, `Gen11`, `g10`).
- **Original-Dateinamen nicht ändern** – die iLO-Firmware wird am Dateinamen (`ilo`) erkannt und zuletzt geflasht.
- Server ohne passenden Unterordner werden übersprungen (im Ergebnis als „Kein Typ-Ordner“ markiert).

Voraussetzungen: **Windows Server 2022**, **PowerShell 7.5+**, iLO-Account mit Update-Berechtigung. iLO 5 (Gen10/Gen10 Plus), iLO 6 (Gen11) bzw. iLO 7 (Gen12).
